      ...
      <div className="flex flex-col lg:flex-row gap-16">
        ...
        <div className="flex-1 space-y-12">
           <div className="flex items-end justify-between border-b border-slate-100 pb-12">
              <h1 className="text-5xl font-black text-slate-900 tracking-tighter leading-none uppercase">Your Bag.</h1>
              <p className="text-slate-400 font-bold text-sm tracking-widest uppercase">{cartItems.length} Items</p>
           </div>
              ...
              {cartItems.length > 0 ? (
                 <div className="space-y-10">
                    {cartItems.map((item) => (
                       ...
                       >
                          <div className="w-full sm:w-48 aspect-square rounded-[2rem] overflow-hidden bg-slate-50 border border-slate-100 flex-shrink-0">
                          ...
                          <div className="flex-1 w-full space-y-6">
                             <div className="flex items-start justify-between gap-4">
                                <div>
                                   ...
                                   <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter leading-tight">{item.name}</h3>
                                   <p className="text-slate-400 font-bold text-sm mt-1 uppercase tracking-widest">In Stock  Stockholm</p>
                                </div>
                                <button 
                                   onClick={() => removeItem(item.id)}
                                   className="p-3 bg-slate-50 rounded-2xl text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-all duration-300"
                                >
                             ...
                             <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                                <div className="inline-flex items-center bg-slate-50 rounded-2xl p-1.5 border border-slate-100/50">
                                   <button 
                                      onClick={() => updateQuantity(item.id, -1)}
                                      className="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-slate-950 hover:bg-white rounded-xl transition-all"
                                   >
                                ...
                                <PriceTag amount={item.price * item.quantity} size="lg" className="font-black text-slate-900" />
                             </div>
                          </div>
                       </motion.div>
                    ))}
           ...
           {cartItems.length > 0 && (
              <Link to="/shop" className="inline-flex items-center gap-2 text-slate-400 hover:text-emerald-600 font-black text-xs uppercase tracking-widest transition-colors group py-8 border-t border-slate-50">
              ...
              <div className="bg-slate-950 rounded-[3rem] p-10 sm:p-12 text-white sticky top-32 shadow-2xl shadow-slate-900/40 overflow-hidden">
                 ...
                 <div className="space-y-8 relative z-10">
                    ...
                    <div className="flex justify-between items-end">
                       <span className="text-slate-400 font-black uppercase text-xs tracking-widest pb-1">Total Due</span>
